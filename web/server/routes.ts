...
export const ROUTES: RouteDef[] = [
  ...
  },
  {
    ...
    summary:
      'Compare ONE span across nodes by correlating it in each node\'s own trace. Runs the search dialect to locate the matching span (give an exact `name` plus an `attr` that pins the operation, e.g. name=round&nameRegex=false&attr=span.height=42), then assembles each match\'s subtree into one multi-instance trace whose lanes share a time axis anchored at the EARLIEST matched span, so each node\'s start skew is visible. Do not force multiple nodes into one deterministic trace id; compare expects one matching span per node trace. The response is the same shape as GET /traces/:id, so the flame/stats/heatmap views render it directly.',
    params: SEARCH_QUERY_PARAMS,
